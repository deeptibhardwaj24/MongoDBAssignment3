...
class EmployeeController extends Controller{
  ...
  def register() = Action { implicit request =>
    addForm.bindFromRequest.fold(
      ...
      badForm => BadRequest(views.html.add(badForm)),
      validForm => {
    val name = validForm._1
        ...
        employeeCollection.insert(BSONDocument("name"-> name, "password"->password, "address"->address,"dob"->dob,"joiningDate"->dateOfJoining,"designation"-> designation ) )
         Thread.sleep(5000)
          Ok("added")
      })}

    def validate = Action(implicit request => loginForm.bindFromRequest.fold(
